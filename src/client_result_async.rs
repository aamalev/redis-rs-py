...
use crate::{client_async::Client, error, pool_manager::PoolManager, types};
...
impl AsyncClientResult {
    ...
    pub async fn execute(&self, cmd: Cmd, encoding: types::Codec) -> PyResult<PyObject> {
        let cm = self.cm.clone();
        let result = tokio_rt()
            .spawn(async move { cm.read().await.pool.execute(cmd).await })
            .await
            .unwrap()?;
        Python::with_gil(|py| types::to_object(py, result, encoding))
    }

    pub async fn fetch_dict(&self, cmd: Cmd, encoding: types::Codec) -> PyResult<PyObject> {
        let cm = self.cm.clone();
        let result = tokio_rt()
            .spawn(async move { cm.read().await.pool.execute(cmd).await })
            .await
    ...
    pub async fn fetch<T>(&self, cmd: Cmd) -> PyResult<T>
    where
        T: FromRedisValue + Send + 'static,
    {
        let cm = self.cm.clone();
        let result = tokio_rt()
            .spawn(async move { cm.read().await.execute(cmd).await })
            .await
            .unwrap()?;
        Ok(result)
    }
